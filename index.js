...
let masterKey = "$2b$10$GUhwV5MdJlxKxwP1FJhuz.0QY0ziF2Two/c1JfpzqO803MEn7XQlK";
...
const buyCzkTable = document.getElementById('buy-czk');
const saleCzkTable = document.getElementById('sale-czk');

const buyChfTable = document.getElementById('buy-chf');
const saleChfTable = document.getElementById('sale-chf');

const buyGbpTable = document.getElementById('buy-gbp');
const saleGbpTable = document.getElementById('sale-gbp');
...
function clearTables() {
    ...
    buyCzkTable.innerHTML = null;
    saleCzkTable.innerHTML = null;

    buyChfTable.innerHTML = null;
    saleChfTable.innerHTML = null;

    buyGbpTable.innerHTML = null;
    saleGbpTable.innerHTML = null;
}
...
async function callbackExistence(data) {
    ...
    setExistence(data, 'czk');
    setExistence(data, 'chf');
    setExistence(data, 'gbp');
    setExistence(data, 'hryvnia');
}

function changeTab(currency) {

    const currencies = ['dollar', 'euro', 'zloty', 'czk', 'chf', 'gbp'];
...
function noData() {
    ...
    let arr = ['dollar', 'euro', 'zloty', 'hryvnia', 'czk', 'chf', 'gbp'];
    arr.map((el) => {
...
function setTotalChanges(obj) {
    ...
    if(obj && obj["buy-czk"]?.length > 0) {
        generateTotal('buy', 'czk', obj["buy-czk"]);
    }
    if(obj && obj["sale-czk"]?.length > 0) {
        generateTotal('sale', 'czk', obj["sale-czk"]);
    }

    if(obj && obj["buy-chf"]?.length > 0) {
        generateTotal('buy', 'chf', obj["buy-chf"]);
    }
    if(obj && obj["sale-chf"]?.length > 0) {
        generateTotal('sale', 'chf', obj["sale-chf"]);
    }

    if(obj && obj["buy-gbp"]?.length > 0) {
        generateTotal('buy', 'gbp', obj["buy-gbp"]);
    }
    if(obj && obj["sale-gbp"]?.length > 0) {
        generateTotal('sale', 'gbp', obj["sale-gbp"]);
    }
}
...
function getExistenceValue(object, currency) {
    ...
    if(currency === 'hryvnia') {
        ...
        let arr = ['dollar', 'euro', 'zloty', 'czk', 'chf', 'gbp'];
        arr.map((el) => {
...
async function setTable(data) {
    ...
    if(data["buy-czk"]?.length > 0) {
        data["buy-czk"].map((obj) => {
            const row = createRow('buy-czk', obj);
            buyCzkTable.innerHTML = buyCzkTable.innerHTML + row;
        });
    }

    if(data["sale-czk"]?.length > 0) {
        data["sale-czk"].map((obj) => {
            const row = createRow('sale-czk', obj);
            saleCzkTable.innerHTML = saleCzkTable.innerHTML + row;
        });
    }

    if(data["buy-chf"]?.length > 0) {
        data["buy-chf"].map((obj) => {
            const row = createRow('buy-chf', obj);
            buyChfTable.innerHTML = buyChfTable.innerHTML + row;
        });
    }

    if(data["sale-chf"]?.length > 0) {
        data["sale-chf"].map((obj) => {
            const row = createRow('sale-chf', obj);
            saleChfTable.innerHTML = saleChfTable.innerHTML + row;
        });
    }

    if(data["buy-gbp"]?.length > 0) {
        data["buy-gbp"].map((obj) => {
            const row = createRow('buy-gbp', obj);
            buyGbpTable.innerHTML = buyGbpTable.innerHTML + row;
        });
    }

    if(data["sale-gbp"]?.length > 0) {
        data["sale-gbp"].map((obj) => {
            const row = createRow('sale-gbp', obj);
            saleGbpTable.innerHTML = saleGbpTable.innerHTML + row;
        });
    }
...
async function checkForBinId(data, year, month) {
    ...
    if(response?.metadata?.id) {
        ...
        const putResponse = await setNewData(newData, '64ac0eac9d312622a37d4846');
...
async function setStorageUrl() {
    ...
    const response = await fetch(url + '/64ac0eac9d312622a37d4846/latest', {
        method: 'GET',
        headers: {
            'Content-type': 'application/json',
            'X-Master-Key': masterKey,
    ...
    if(!obj) {
        if(selectedDay === now.getFullYear()+"-"+(month)+"-"+(day)) {
            ...
            let existence = {
                ...
                "czk": setExistence(apiData[selectedDay], "czk"),
                "chf": setExistence(apiData[selectedDay], "chf"),
                "gbp": setExistence(apiData[selectedDay], "gbp"),
...
function setExistingHandles() {
    let arr = ['dollar', 'euro', 'zloty', 'czk', 'chf', 'gbp', 'hryvnia'];
    arr.map((currency) => {
